...
describe("app view model", () => {
	it("tracks connection state and host events", () => {
		const connecting = reduceAppViewState(
			createInitialAppViewState("http://localhost:4739"),
			{
				type: "connecting",
			},
		);
		const connected = reduceAppViewState(connecting, {
			type: "connected",
			status: hostStatus,
		});

		expect(connected.connectionState).toBe("connected");
		expect(connected.client.connectionMessage).toBe(
			"Mac on darwin · Pi coding agent v0.78.1",
		);
	});

	it("seeds the workspace path from the host cwd when the current path is not absolute", () => {
		const relative = reduceAppViewState(
			createInitialAppViewState("http://localhost:4739"),
			{
				type: "setCwd",
				value: ".",
		...
		expect(
			reduceAppViewState(relative, { type: "connected", status: hostStatus })
				.cwd,
		).toBe("/tmp/project");
	});

	it("keeps an explicitly absolute workspace path when connecting", () => {
		const absolute = reduceAppViewState(
			createInitialAppViewState("http://localhost:4739"),
			{
				type: "setCwd",
				value: "/tmp/other",
		...
		expect(
			reduceAppViewState(absolute, { type: "connected", status: hostStatus })
				.cwd,
		).toBe("/tmp/other");
	});

	it("keeps an explicit tilde workspace path when connecting", () => {
		const tilde = reduceAppViewState(
			createInitialAppViewState("http://localhost:4739"),
			{
				type: "setCwd",
		...
		expect(
			reduceAppViewState(tilde, { type: "connected", status: hostStatus }).cwd,
		).toBe("~/Documents/project");
	});

	it("switches to the session screen after a session opens", () => {
		const connected = reduceAppViewState(
			createInitialAppViewState("http://localhost:4739"),
			{
				type: "connected",
				status: hostStatus,
	...
	it("toggles session header collapse state", () => {
		const collapsed = reduceAppViewState(
			createInitialAppViewState("http://localhost:4739"),
			{
		...
		expect(
			reduceAppViewState(collapsed, { type: "toggleSessionHeader" })
				.sessionHeaderCollapsed,
		).toBe(false);
	});

	it("sets non-connection errors", () => {
		const state = reduceAppViewState(
			createInitialAppViewState("http://localhost:4739"),
			{
				type: "setError",
