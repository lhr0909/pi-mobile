...
import { HostClient, normalizeBaseUrl, toWebSocketUrl } from "../src/host-client.js";
...
describe("host client URL helpers", () => {
  it("normalizes base URLs", () => {
    expect(normalizeBaseUrl(" http://127.0.0.1:4739/ ")).toBe("http://127.0.0.1:4739");
  });
  ...
  it("builds websocket URLs with optional token", () => {
    ...
    expect(toWebSocketUrl("https://example.test")).toBe("wss://example.test/ws");
  });
});
...
describe("HostClient", () => {
  it("sends bearer auth and parses host status", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(
        JSON.stringify({ name: "Mac", version: "0.1.0", platform: "darwin", cwd: "/tmp", pid: 1, sdkMode: "sdk" }),
        { status: 200, headers: { "content-type": "application/json" } },
      ),
    );

    const client = new HostClient({ baseUrl: "http://host.test", token: "secret" });
  ...
  it("opens sessions and sends prompt commands", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ session: { id: "s1" }, timeline: [], nextSeq: 1 }), {
          status: 201,
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({ method: "POST", body: JSON.stringify({ cwd: "/tmp/project", mode: "new" }) }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
  ...
  it("surfaces API errors", async () => {
    ...
    await expect(new HostClient({ baseUrl: "http://host.test" }).status()).rejects.toThrow("Unauthorized");
  });
