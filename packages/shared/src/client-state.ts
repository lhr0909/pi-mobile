...
export function reduceHostEvent(state: MobileClientState, event: HostEvent): MobileClientState {
  switch (event.type) {
    ...
    case "timeline_item":
      return updateTimeline(state, event.sessionId, timeline => upsertTimelineItem(timeline, event.item));
    case "timeline_delta":
      return updateTimeline(state, event.sessionId, timeline => appendTimelineDelta(timeline, event.itemId, event.delta));
    case "command_error":
...
function upsertSession(
  ...
  return {
    ...state,
    ...(resolvedActiveSessionId ? { activeSessionId: resolvedActiveSessionId } : {}),
    sessions: {
      ...state.sessions,
...
function updateSessionState(state: MobileClientState, session: SessionState): MobileClientState {
...
function updateTimeline(
  ...
  return upsertSession(state, { ...existing, timeline: update(existing.timeline) }, state.activeSessionId);
}

function upsertTimelineItem(timeline: TimelineItem[], item: TimelineItem): TimelineItem[] {
  const existingIndex = timeline.findIndex(existing => existing.id === item.id);
  if (existingIndex === -1) {
    return [...timeline, item];
  }

  return timeline.map(existing => (existing.id === item.id ? item : existing));
}
